...
def test_zone_inexistante():
    ...
    """
    assert evolution_par_decennie('Nîmes', donnees_test) == {}
    assert donnees_test == {}
...
def test_une_seule_decennie():
    ...
    """
    assert evolution_par_decennie("Marquises", donnees_test)
    assert len(donnees_test) == 1
    assert 2020 in donnees_test
    assert donnees_test[2020] == 26.0
...
def test_plusieurs_decennies():
    ...
    """
    assert evolution_par_decennie("Societe", donnees_test)
    assert 2010 in donnees_test
    assert 2020 in donnees_test
    assert donnees_test[2010] > 26
    assert donnees_test[2020] > donnees_test[2010]
...
def evolution_par_decennie(zone, donnees):
    ...
    for releve in donnees:
        if releve['zone'] == zone:
            annee = int(releve['date'][:4])
            decennie = (annee // 10) * 10
            if decennie not in resultats:
                resultats[decennie] = []
            resultats[decennie].append(releve['temperature'])
    for decennie in resultats:
        temp_list = resultats[decennie]
        resultats[decennie] = sum(temp_list) / len(temp_list)
    return resultats