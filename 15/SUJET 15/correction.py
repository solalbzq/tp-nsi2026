...
def normalisation_tel(num):
    ...
    :param num: (str) numéro de telephone
    :return (str) numero de telephone avec les chiffres seul
    """
    assert isinstance(num,str), "doit etre une chaine de caractere"
    bon_num = ''
    for c in num:
        if c.isdigit():
            bon_num += c
...
def consultation_vaccination_chat(date):
    ...
    WHERE consultation.date > ? AND consultation.motif LIKE "vaccination" AND animal.espece LIKE "chat"
