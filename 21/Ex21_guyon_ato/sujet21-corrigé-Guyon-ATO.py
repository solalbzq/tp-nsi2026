...
def extraire_cartes_du_jour(paquet, date_jour):
    cartesDuJour = []
    for carte in paquet:
        print(carte.date_prochaine)
        if str(carte.date_prochaine)[:3] <= date_jour[:3]: # on vérifie l'année
            if str(carte.date_prochaine)[5::6] <= date_jour[5::6]: # on vérifie le mois
                if str(carte.date_prochaine)[8:] <= date_jour[8:]: # on vérifie le jour
...
def extraire_cartes_a_renforcer(paquet):
    ...
    for carte in paquet:
        if carte.niveau < niveau_min:
            ...
            a_renforcer.clear() # correction de la fonction extraire_cartes_a_renforcer pour la question 3
